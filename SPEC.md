...
```
1단계: 모델 페이지 접근
    GET https://www.samsung.com/sec/support/model/{model}/
    → HTML에서 goodsId 추출 (정규식 6개 패턴)

2단계: 매뉴얼 API 호출 (goodsId 방식)
    POST https://www.samsung.com/sec/xhr/goods/goodsManual
    Body: {mdlCode, goodsId, manualLang: "KO", supportYn: "Y"}
    → 응답 HTML에서 PDF 링크 파싱

3단계: Fallback — 모델 페이지 직접 탐색
    CSS 셀렉터로 PDF 링크 직접 추출
    → 실패 시 미국 사이트(samsung.com/us) 시도
