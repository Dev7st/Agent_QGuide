...
```
Samsung 매뉴얼은 비공개 내부 API(goodsId 방식)로 PDF 링크를 획득한다.
이전 프로젝트(samsung_crawler.py) 분석으로 확인된 실제 동작 엔드포인트를 사용한다.
```

### 매뉴얼 URL 탐색 순서

```
공통: GET https://www.samsung.com/sec/support/model/{model}/ → HTML 1회 요청

1단계: 모델 페이지 HTML에서 CSS 셀렉터 + 정규식으로 PDF 링크 직접 탐색
    → 찾으면 즉시 반환

2단계: goodsId 추출 → 매뉴얼 API POST (동적 로딩 대응)
    PDF 링크가 JavaScript로 동적 로딩될 때 사용
    POST https://www.samsung.com/sec/xhr/goods/goodsManual
    Body: {mdlCode, goodsId, manualLang: "KO", supportYn: "Y"}
    → 응답 HTML에서 PDF 링크 파싱 → 찾으면 반환

3단계: Fallback — 미국 사이트 시도
    GET https://www.samsung.com/us/support/{model}/
    → CSS 셀렉터로 PDF 링크 탐색
    → 최종 실패 시 None 반환 → crawl() False 반환
```
...
```
Samsung 매뉴얼 PDF는 텍스트 레이어가 포함된 텍스트 기반 PDF
→ PyMuPDF(fitz)로 직접 텍스트 추출 (OCR 불필요)

이미지 기반 PDF(스캔본)는 구현 범위 외
→ 텍스트 추출 결과가 없으면 크롤링 실패(False) 처리
```
...
PDF URL
  ↓
PyMuPDF(fitz)로 다운로드 및 분석
  ├── 텍스트 레이어 있음 → get_text()로 직접 추출 → 청킹 → ChromaDB/ES 저장
  └── 텍스트 없음 (이미지 기반) → 처리 불가 → False 반환
```
